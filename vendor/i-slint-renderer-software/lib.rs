...
impl<T: ProcessScene> sharedparley::GlyphRenderer for SceneBuilder<'_, T> {
    ...
    fn draw_glyph_run(
        ...
        // Keep text positions wide until after clipping. TextInput lays out the
        // entire document, including glyphs far outside the visible viewport.
        // Casting those positions to i16 first overflows for documents that are
        // a few thousand lines long, even though the glyphs would be clipped.
        ...
        for positioned_glyph in glyphs_it {
            ...
            let gl_y = euclid::Length::<f32, PhysicalPx>::new(glyph.y.truncate() as f32);
            let glyph_height = glyph.height.cast::<f32>();
            let target_rect = euclid::Rect::<f32, PhysicalPx>::new(
                (euclid::Point2D::<f32, PhysicalPx>::from_lengths(
                    euclid::Length::new(0.0),
                    -gl_y - glyph_height,
                )
                    + global_offset
                    + glyph_offset)
            ...
            let physical_clip: euclid::Rect<f32, PhysicalPx> =
                (self.current_state.clip.translate(self.current_state.offset.to_vector()).cast()
                    * self.scale_factor)
                    .round()
                    .transformed(self.rotation);

            if !target_rect.intersects(&physical_clip) {
                continue;
            }
            let target_rect: PhysicalRect = target_rect.cast();

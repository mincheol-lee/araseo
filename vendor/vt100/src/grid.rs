...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // A top-anchored region is used by inline TUIs to keep their
            // prompt fixed while pushing transcript rows into history.
            // Preserve those rows just like a full-screen scroll. Regions
            // starting below row zero still must not enter scrollback.
